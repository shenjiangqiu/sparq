...
def truncate_f32_tensor(tensor: torch.Tensor, num_valid_bits: int) -> torch.Tensor:
    ...
    """
    assert (
        tensor.dtype == torch.float32 or tensor.dtype == torch.float16
    ), "Input tensor must be of dtype torch.float32 or torch.float16."
...
class LlamaAttentionWithANN(llama_attention.LlamaAttention):
    def __init__(
        ...
        self.settings = settings
        self.expatt = ExpAttention(settings, self.num_key_value_heads, self.head_dim)
...
class GemmaAttentionWithANN(gemma_attention.GemmaAttention):
    def __init__(
        ...
        super().__init__(config, layer_idx)
        self.settings = settings
        self.expatt = ExpAttention(settings, self.num_heads, self.head_dim)
